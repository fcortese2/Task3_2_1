...
namespace GADE6112_POE
{
    ...
    {
        public MeleeUnit(int _xPos, int _yPos, float _health, int _speed, int _attack, int _attackRange, int _teamID, string _symbol, string _name)  //constructor
        {
            this.xPos = _xPos;
            this.yPos = _yPos;
            this.health = _health;
        ...
        public override void MoveUnit(ref Unit FindClosest)          //A method to move to a new position;
        {
            ...
            if (FindClosest.Team() != teamID)
            {
                if (health < 25)
                {
                    ...
                    xPos = Convert.ToInt32(Clamp(xPos, 20));        //check values in range
        ...
        {
            throw new DeathException(this.ToString() + "IS DEAD");     //kill unit with new system exception
        }
        ...
        private float Clamp(float value, float max, float min = 0)  //keep value within range
        {
            float Clamped = value;
            if (value < min)
            {
        ...
        public class DeathException : System.Exception            //new System Exc for unit death
        {
